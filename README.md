...
#### open_in_viewer
Open a MODFLOW 6 model that was built and run on your machine in the MODFLOW AI web viewer: mesh, packages, heads per timestep, cell inspector, cross section, 3D. The assistant installs the writer once (`python -m pip install mfai-viewer`, on [PyPI](https://pypi.org/project/mfai-viewer/), Python 3.10 or newer; pip brings numpy, flopy, flatbuffers, pydantic, scipy, matplotlib and shapely), runs `mfai-viewer snapshot` on the model directory, sends the small `summary.json` it prints to this tool, and receives one `mfai-viewer upload` command that sends the files, completes the link and prints the URL. Nothing is downloaded into your project directory.
